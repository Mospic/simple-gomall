...
func Register(ginCtx *gin.Context) {
	var userReq user.MallUserRegisterRequest
	//获取用户名和密码
	...
	//返回
	ginCtx.JSON(http.StatusOK, user.MallUserRegisterResponse{
...
func Login(ginCtx *gin.Context) {
	var userReq user.MallUserLoginRequest
	userReq.Username = ginCtx.Query("username")
	userReq.Password = ginCtx.Query("password")
	...
	ginCtx.JSON(http.StatusOK, user.MallUserLoginResponse{
		StatusCode: userResp.StatusCode,
		StatusMsg:  userResp.StatusMsg,
		UserId:     userResp.UserId,
		Token:      token,
...
func UserInfo(ginCtx *gin.Context) {
	var userReq user.MallUserRequest
	...
	ginCtx.JSON(http.StatusOK, user.MallUserResponse{
		StatusCode: userResp.StatusCode,
		StatusMsg:  userResp.StatusMsg,
		User:       userResp.User,
	})
